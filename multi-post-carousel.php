...
add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), 'multi_post_carousel_plugin_action_links' );

function multi_post_carousel_plugin_action_links( $links ) {
if(is_admin()){
   $links[] = '<a href="'. esc_url( get_admin_url(null, 'options-general.php?page=post-slide') ) .'" target="_blank">Settings</a>';
    ...
    for ($i=1; $i <=$slide_count; $i++) :
     ...
     $ps_query = new WP_Query( $args2 );
      ...
      if ( $ps_query->have_posts() ) { 
		...
		$output .= '<div class="mpc-content-area"><div class="single-slide-img"><a href="'.get_permalink().'"><img src="'.$full_img_url.'" /></a></div><div class="mpc-title" style="display:'.$display_title.'"><a href="'.get_permalink().'">'.wp_trim_words( get_the_title(), $ps_options['title_word_limit'], " " ).'</a></div><div class="mpc-content" style="display:'.$display_content.'">'.wp_trim_words( get_the_excerpt(), $ps_options['content_word_limit'], " " ).'</div><div class="mpc-read-btn" style="display:'.$display_read_more.'"><a href="'.get_permalink().'">Read More</a></div></div>';
        $skip++;
        endwhile;// while end
